...
from transformer_libs import decoder_weight_tying
import random
import numpy as np
...
def create_weight_tie_model(LOAD, directory, model_params, train_params, lr_params, file=None):
    ...
    if LOAD:
        ...
        # Load the model, optimizer, and model parameters from the specified file
        model, opt, model_params, train_params, lr_params = load_weight_tie_model(file, model_params, train_params, lr_params)
    else:
        # If LOAD is set to False, generate a new model to train
        print('Generating a new model to train.')
        model, opt, model_params, train_params, lr_params = default_weight_tie_model(model_params, train_params, lr_params)
        train_params['batch_num'] = 0
        train_params['samples_done'] = 0
...
def default_weight_tie_model(model_params, train_params, lr_params):
    ...
    model_params['samples_done'] = 0
    model_params['batch_num'] = 0
    model = decoder_weight_tying.DecoderWeightTie(num_blocks, d_model, d_middle, vocab_size, dropout, h, d_q, d_k, d_v, weight_tying)
    model.cuda()
...
def load_weight_tie_model(file, model_params, train_params, lr_params, cuda=True):
    ...
    model = decoder_weight_tying.DecoderWeightTie(num_blocks, d_model, d_middle, vocab_size, dropout, h, d_q, d_k, d_v, use_weight_tying=True)
