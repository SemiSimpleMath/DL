...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        return self.l2(self.dropout(F.relu(self.l1(x))))  # added drop out, dropout could be after l2?
...
class MultiHeadAttentionModule(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        return self.linear(torch.cat([layer(q, k, v) for layer in self.a_modules], dim=-1)) # could do dropout here
                                                            # currently dropout is done after this layer
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.dropout1(self.multi_head_masked(normed_x, normed_x, normed_x)) # if the dropout is moved then its not needed here
