...
class DecoderWeightTie(nn.Module):
    def __init__(self, num_blocks, d_model, d_middle, d_token, dropout, h, d_Q, d_K, d_V, use_weight_tying=False):
        ...
        initial_module = self.layers[0].multi_head_masked.l_proj
        for i, layer in enumerate(self.layers):
            if i != 0:
                layer.multi_head_masked.l_proj.weight = nn.Parameter(initial_module.weight.data, requires_grad=False)
