...
def train(model, opt, ds, tok, loss_func, bs, num_batches, seq_len, model_params):
    ...
    lr_scale_factor = (1+np.log(config.accumulate_size))
    ...
    while batch_num < start_batch_num + num_batches:
        # load sample
        combined = data_utils.get_wiki_batch(ds, tok, bs, samples_done, seq_len + 1)  # combined is bs x (L + 1)
        samples_done += 1
...
def main():
    ...
    utils.print_model_params(model_params)
